...
check_homogenous <- function(
  ...
  if (
    strict &&
      (!identical(union, intersect) ||
        !all(vapply(classes, identical, TRUE, union)))
  ) {
    chk_fail(
      "elements in ",
      x_name,
    ...
    chk_fail(
      "all elements in ",
      x_name,
      " must inherit from the same class",
      error = error
    )
    return(invisible(x))
  }
  ...
  if (recursive) {
    lapply(
      ...
      strict = strict,
      recursive = recursive,
      x_name = paste("elements in", x_name),
      error = error
    )
  }
  invisible(x)
