...
check_dbl <- function(x, coerce = FALSE, x_name = substitute(x), error = TRUE) {
...
check_pos_dbl <- function(
  ...
  if (x <= 0) {
    chk_fail(x_name, " must be positive", error = error)
  }
  invisible(x)
}
...
check_noneg_dbl <- function(
  ...
  check_scalar(x, 1, x_name = x_name, error = error)
  if (x < 0) {
    chk_fail(x_name, " must be non-negative", error = error)
  }
  invisible(x)
}
...
check_neg_dbl <- function(
  ...
  check_scalar(x, 1, x_name = x_name, error = error)
  if (x >= 0) {
    chk_fail(x_name, " must be negative", error = error)
  }
  invisible(x)
}
