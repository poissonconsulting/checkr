...
check_attributes <- function(
  ...
  if (!length(attr)) {
    chk_fail(x_name, " must have attributes", error = error)
  }
  ...
  if (!is.null(attr) && !is.null(values)) {
    check_list(
      ...
      order = order,
      exclusive = exclusive,
      x_name = paste("attributes of", x_name),
      error = error
    )
  }
...
check_no_attributes <- function(
  ...
  if (length(attr)) {
    chk_fail(x_name, " must not have attributes", error = error)
  }
