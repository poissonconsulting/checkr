...
check_vector_values <- function(x,
  ...
  check_vector_class(x, values, x_name = x_name, error = error)
  check_vector_nas(x, values, x_name = x_name, error = error)
...
#'
#' @param x The object to check.
#' @param values A named list specifying the values.
#' @param x_name A string of the name of the object.
#' @param error A flag indicating whether to throw an informative error or immediately generate an informative message if the check fails.
#' @return An invisible copy of x (if it doesn't throw an error).
#'
#' @examples
#' check_list_values(list(x = 2), values = list(x = 1), error = FALSE)
check_list_values <- function(x,
                              values,
                              x_name = substitute(x),
                              error = getOption("vectorcheckr.error", TRUE)) {
  if (!is.character(x_name)) x_name <- deparse(x_name)
  ...
  check_list_internal(values)
  check_names(values, )

  check_list_class(x, values, x_name = x_name, error = error)
  check_list_nas(x, values, x_name = x_name, error = error)
