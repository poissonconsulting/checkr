...
check_colnames <- function(x, names, exclusive = FALSE, order = FALSE,
  ...
  if (exclusive) {
    if (order) {
      if (!identical(x_names, names))
        on_fail(x_name, " column names must be identical to ", punctuate(names, "and"), error = error)
    } else {
      if (!identical(sort(x_names), sort(names)))
        on_fail(x_name, " column names must include and only include ", punctuate(names, "and"), error = error)
    }
  } else {
    x_names <- x_names[x_names %in% names]
    if (order) {
      if (!identical(x_names, names))
        on_fail(x_name, " column names must include ", punctuate(names, "and"), " in that order", error = error)
    } else {
      if (!identical(sort(x_names), sort(names)))
        on_fail(x_name, " column names must include ", punctuate(names, "and"), error = error)
    }
  }
  invisible(x)
}
