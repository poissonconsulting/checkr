...
check_nas <- function(x, values, x_name = substitute(x), error = TRUE) {
...
check_values <- function(
  ...
  if (!is.atomic(x)) {
    err(x_name, " must be an atomic vector")
  }
  if (!is.atomic(values)) {
    err("values must be an atomic vector")
  }
