...
It is implemented by issuing an informative error message if the function arguments fail satisfy specific criteria.
This is particularly important in R because it is not a compiled language.
...
The [`checkr`](https://github.com/poissonconsulting/checkr) package provides a handful of expressive functions to test the values of objects.
...
A speed comparison would be interesting.

### Dependencies

Whereas the `checkmate` package depends on `backports` and `utils`,
and `assertthat` depends on `tools` both `base` and `checkr` are dependency-free.
