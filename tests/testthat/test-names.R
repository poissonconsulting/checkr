test_that("names", {
  ...
  expect_error(
    check_names(vec, c("y", "x"), exclusive = TRUE),
    "vec names must not include 'z'"
  )
  expect_error(
    check_names(vec, c("y", "x"), order = TRUE),
    "vec names must include 'y' and 'x' in that order"
  )
  expect_error(check_names(vec, c("a")), "vec names must include 'a'")

  expect_identical(check_names(vec), vec)
  expect_error(
    check_names(vec, exclusive = TRUE),
    "vec must not have any elements"
  )
  ...
  expect_identical(check_names(vec), vec)
  expect_identical(
    check_names(vec, c("x", "y", "z", "z"), order = TRUE, exclusive = TRUE),
    vec
  )
  expect_error(
    check_names(vec, c("x", "y", "z", "z"), unique = TRUE),
