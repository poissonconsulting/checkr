test_that("levels", {
  ...
  expect_error(
    check_levels(factor(1), c("y", "x")),
    "factor[(]1[)] levels must be identical to 'y' and 'x'"
  )
...
test_that("levels errors for order and exclusive args", {
  ...
  expect_error(
    check_levels(x, exclusive = FALSE, levels = c("b", "c", "a")),
    "x levels must include 'b', 'c' and 'a' in that order"
  )

  expect_error(
    check_levels(x, order = FALSE, levels = c("b", "c", "a", "d")),
    "x levels must include and only include 'b', 'c', 'a' and 'd'"
  )

  expect_error(
    check_levels(x, levels = c("b", "c", "a")),
