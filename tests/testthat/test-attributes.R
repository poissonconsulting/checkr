test_that("check_attributes", {
  ...
  expect_error(
    check_attributes(x, values = list(y = 3:4)),
  ...
  expect_error(
    check_attributes(y, class = FALSE, names = FALSE),
    "y must have attributes"
  )
})
...
test_that("check_no_attributes", {
  ...
  attributes(x) <- list(y = 2L)
  expect_error(check_no_attributes(x), "x must not have attributes")
  y <- 1
  ...
  class(y) <- c("sn", "numeric")
  expect_error(
    check_no_attributes(y, names = FALSE),
    "y must not have attributes"
  )
  expect_error(
    check_no_attributes(y, class = FALSE),
    "y must not have attributes"
  )
