test_that("length1", {
  ...
  expect_error(
    check_homogenous(z, strict = TRUE),
    "elements in z must have identical classes"
  )
  class(y) <- c("a")
  z <- list(x, y)
  expect_identical(check_homogenous(z), z)
  ...
  expect_identical(check_homogenous(zz, strict = TRUE), zz)
  expect_error(
    check_homogenous(zz, strict = TRUE, recursive = TRUE),
    "elements in elements in zz must have identical classes"
  )

  expect_identical(check_homogenous(datasets::mtcars), datasets::mtcars)
  expect_error(
    check_homogenous(datasets::ChickWeight),
    "all elements in datasets::ChickWeight must inherit from the same class"
  )
})
